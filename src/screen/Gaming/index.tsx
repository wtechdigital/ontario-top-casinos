  ...
  return (
    <WSpacedContainer className="xsm:px-4 py-8">
     {!loading ?  <div>
      ...
      <h1 className="text-[22px]  leading-[52px] xsm:text-10 p-4">
        {responsibleGaming?.data[0]?.attributes?.description}
      </h1>
      <ul className="grid !gap-y-[16px] text-base mt-10 p-5">
        {responsibleGaming?.data[0]?.attributes?.points?.map(
          (point: any, index: number) =>
            point?.children?.map((child: any, childIndex: number) => (
              <div key={index} className="flex items-start ">
               <span className="text-[22px] pb-4" key={childIndex}>{child?.text}</span>
             </div>
            ))
        )}
      </ul>
